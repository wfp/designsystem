...
.addWithInfo(
  ...
  () => {
    const colorList = colors.ui_colors.map((color) =>
     <li style={{display: 'flex', webkitColumnBreakInside: 'avoid', pageBreakInside: 'avoid', breakInside: 'avoid-column'}}>
      <div style={{width: '2.5em', height: '2.5em', marginTop: '0.5em', marginRight: '1em', backgroundColor: color.hex}}></div>
      <div>
        <h4>{color.name}</h4>
        <p>js: {color.name}</p>
        <p>scss: {color.scss}</p>
