...
.addWithInfo(
  ...
  () => (
    <Page title="Form UX-Design" subTitle="Recommendations for form design">
      ...
      <p>Especially longer forms should provide autosave by default. You should also provide a very clear feedback by displaying a message that shows when the last autosave happened or a Save-Button. The user should be always able to also save forms with an error.</p>
      ...
      <p>The only exception to this rule is when the input needs multiple field elements like asking for dates (day, month, year) or time (hours and minutes), where multiple fields are expected to be on one line. <Link href="https://uxplanet.org/the-18-must-do-principles-in-the-form-design-fe89d0127c92">read more</Link></p>

      <h3>Group related fields together into sections or steps</h3>

      <p>If your form has more than six fields, it’s considered good practice to group questions into logical sections or steps. Use &#x3C;FormGroup /&#x3E; for this behaviour.</p>
...
And, more importantly, always try to explain the root of the problem. Try to use a human language and explain what exactly the user/the system made wrong, and what exactly should be fixed/filled.
  ...
  <p>Always ask the user before submitting input. Do not ask if the user only saves the input.</p>
      <ul>
        <li>
