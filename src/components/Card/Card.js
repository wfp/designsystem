...
const Card = ({ children, className, image, isExternal, isLink, metadata, more, subTitle, title, kind, url, ...other }) => {
    ...
    <React.Fragment>
      ...
      <div className="wfp--photo-card__info">
        <div>
          ...
          {metadata &&
            <p className="wfp--photo-card__info__metadata">
              {metadata}
            </p>
          }
        ...
        {kind === 'hero' || kind === 'splash-image' || kind === 'splash-compact'  &&
          <div className="wfp--photo-card__info__more">
            {more}
          </div>
        }
      </div>
