...
keys.forEach(function(element, i) {
  ...
  const additionalName = split[split.length-1].replace('.hbs', '').replace(split[split.length-2] + '-', ' ');
  ...
  .addDecorator(story => <HtmlComponent html={htmlData[i]}>{story()}</HtmlComponent>)
  .add(
    'html' + additionalName,
    (() => null)
  );
