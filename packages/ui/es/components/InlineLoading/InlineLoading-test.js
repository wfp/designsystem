...
describe('Loading', function () {
  describe('Default state renders as expected', function () {
    ...
    var container = wrapper.find('.wfp--inline-loading');
    ...
    it('container has the expected classes', function () {
      expect(container.hasClass('wfp--inline-loading')).toEqual(true);
    });
    it('should add extra classes that are passed via className', function () {
      expect(container.hasClass('extra-class')).toEqual(true);
    });
    it('should render an animation container', function () {
      expect(wrapper.find('.wfp--inline-loading__animation').length).toEqual(1);
    });
    it('should not render any text', function () {
      expect(wrapper.find('.wfp--inline-loading__text').length).toEqual(0);
    });
    it('should not render the SUCCESS state', function () {
      expect(wrapper.find('.wfp--inline-loading__checkmark-container').length).toEqual(0);
    });
  });
  describe('Text rendered as expected', function () {
    ...
    it('should render the provided description', function () {
      expect(wrapper.find('.wfp--inline-loading__text').length).toEqual(1);
      expect(wrapper.find('.wfp--inline-loading__text').text()).toEqual('Loading Things...');
    });
  });
  describe('Success state should render properly', function () {
    ...
    it('should render the success animation', function () {
      expect(wrapper.find('.wfp--inline-loading__checkmark-container').length).toEqual(1);
    });
    it('should render the checkmark within the success animation', function () {
      expect(wrapper.find('.wfp--inline-loading__checkmark').length).toEqual(1);
    });
