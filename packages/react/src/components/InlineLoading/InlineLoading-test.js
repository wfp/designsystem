...
describe('Loading', () => {
  describe('Default state renders as expected', () => {
    ...
    const container = wrapper.find('.wfp--inline-loading');
    ...
    it('should not render the SUCCESS state', () => {
      expect(
        wrapper.find('.wfp--inline-loading__checkmark-container').length
      ).toEqual(0);
    });
  });
  ...
  describe('Text rendered as expected', () => {
    ...
    it('should render the provided description', () => {
      ...
      expect(wrapper.find('.wfp--inline-loading__text').text()).toEqual(
        'Loading Things...'
      );
    });
  });
